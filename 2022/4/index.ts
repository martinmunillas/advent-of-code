const containsRange = (main: TwoNumbers, child: TwoNumbers) => {
  return child[0] >= main[0] && child[1] <= main[1];
};

type TwoNumbers = [number, number];

export const getContainingSectionsCountPartA = (input: string): number => {
...
export const getContainingSectionsCountPartB = (input: string): number => {
  let currentRanges: number[] = [];
  let result = 0;
  let currentNumber = "";
  for (let i = 0; i <= input.length; i++) {
