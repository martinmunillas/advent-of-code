export const getTopOfFinalStacksPartA = (input: string): string => {
...
export const getTopOfFinalStacksPartB = (input: string): string => {
  const stacks: Record<number, string[]> = {};
  let stacksComplete = false;
  let stackCount = 0;
    ...
    if (stacksComplete) {
      if (char === "\n" || char === undefined) {
        ...
        for (let j = 0; j < numbers[0]; j++) {
          const toPush = stacks[numbers[1]].pop();
          if (toPush) {
            subStack.push(toPush);
          }
        }
        for (let j = 0; j < subStack.length; j++) {
          const toPush = subStack[subStack.length - (1 + j)];
          if (toPush) {
            stacks[numbers[2]].push(toPush);
          }
        }
